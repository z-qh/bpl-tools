...
template <typename PointT, typename SemanticT> int SemanticIterativeClosestPoint<PointT, SemanticT>::align(SemanticCloudPtr finalCloud) {
    Eigen::Matrix4d mat = Eigen::Matrix4d::Identity();
    Sophus::SE3d init(mat);
    return align(finalCloud, init);
};

template <typename PointT, typename SemanticT>
int SemanticIterativeClosestPoint<PointT, SemanticT>::align(SemanticCloudPtr finalCloud, Sophus::SE3d& initTransform) {
    ...
    while (converged != true) {
        ...
        problem.AddParameterBlock(estTransform.data(), Sophus::SE3d::num_parameters, new LocalParameterizationSE3);
        ...
        for (SemanticT s : sourceCloud_->semanticLabels) {
            ...
            if (targetCloud_->labeledPointClouds.find(s) != targetCloud_->labeledPointClouds.end()) {
                if (sourceCloud_->labeledPointClouds[s]->size() > 5) {
                    ...
                    pcl::transformPointCloud(*(sourceCloud_->labeledPointClouds[s]), *transformedSource, transMat);
                    ...
                    for (int sourceIndx = 0; sourceIndx < (int)transformedSource->size(); sourceIndx++) {
                        ...
                        if (distSq[0] < 1) {
                            ...
                            ceres::CostFunction* cost_function =
                                new GICPCostFunction(sourcePoint, targetPoint, sourceCov, targetCov, baseTransformation_);
                            problem.AddResidualBlock(cost_function, new ceres::CauchyLoss(1.5), estTransform.data());
                            ...
                            if (false) {
                                ...
                                ceres::GradientChecker gradient_checker(cost_function, &lp, numeric_diff_options);
...
template <typename PointT, typename SemanticT>
Sophus::SE3d SemanticIterativeClosestPoint<PointT, SemanticT>::iterativeMean(std::vector<Sophus::SE3d> const& in, size_t maxIterations) {
    size_t N = in.size();
...
struct PoseFusionCostFunctor {
    PoseFusionCostFunctor(Sophus::SE3d pose, Eigen::Matrix<double, 6, 6> cov)
        : poseInv_(pose)
        , covInv_(cov) {}

    template <class T> bool operator()(T const* const parameters, T* residuals) const {
...
template <typename PointT, typename SemanticT>
Sophus::SE3d SemanticIterativeClosestPoint<PointT, SemanticT>::poseFusion(std::vector<Sophus::SE3d> const& poses, CovarianceVector const& covs,
    ...
    problem.AddParameterBlock(fusedPose.data(), Sophus::SE3d::num_parameters, new LocalParameterizationSE3);
    ...
    for (size_t n = 0; n < poses.size(); n++) {
        // std::cout << poses[n].matrix() << std::endl;
        // std::cout << covs[n] << std::endl;
        PoseFusionCostFunctor* c = new PoseFusionCostFunctor(poses[n].inverse(), covs[n].inverse() * scale);

        ceres::CostFunction* costFunction = new ceres::AutoDiffCostFunction<PoseFusionCostFunctor, 1, Sophus::SE3d::num_parameters>(c);

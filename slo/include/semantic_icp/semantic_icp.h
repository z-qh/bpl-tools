...
template <typename PointT, typename SemanticT> class SemanticIterativeClosestPoint {
public:
    ...
    typedef std::vector<Eigen::Matrix<double, 6, 6>, Eigen::aligned_allocator<Eigen::Matrix<double, 6, 6>>> CovarianceVector;
    ...
    inline void setInputSource(const SemanticCloudPtr& cloud) { sourceCloud_ = cloud; };

    inline void setInputTarget(const SemanticCloudPtr& cloud) { targetCloud_ = cloud; };

    int align(SemanticCloudPtr finalCloud);

    int align(SemanticCloudPtr finalCloud, Sophus::SE3d& initTransform);
...
protected:
    ...
    Sophus::SE3d poseFusion(std::vector<Sophus::SE3d> const& poses, CovarianceVector const& covs, Sophus::SE3d const& initTransform);
};

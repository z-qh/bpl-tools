...
private:
    ...
    detail_msgs::cloud_info segInfo;
...
private:
    ...
    void checkSystemInitialization(sensor_msgs::PointCloud2& laser_cloud_corner_last,
                                   sensor_msgs::PointCloud2& laser_cloud_surf_last)
    {
        ...
        laser_cloud_corner_last = laserCloudCornerLast2;

        sensor_msgs::PointCloud2 laserCloudSurfLast2;
        pcl::toROSMsg(*laserCloudSurfLast, laserCloudSurfLast2);
        laserCloudSurfLast2.header.stamp = cloudHeader.stamp;
        laserCloudSurfLast2.header.frame_id = "map";
        laser_cloud_surf_last = laserCloudSurfLast2;
    ...
    void publishCloudsLast(sensor_msgs::PointCloud2& laser_cloud_corner_last,
                           sensor_msgs::PointCloud2& laser_cloud_surf_last,
                           sensor_msgs::PointCloud2& outlier_cloud_last)
    {
        ...
        if (frameCount >= skipFrameNum + 1) {
            ...
            outlierCloudLast2.header.frame_id = "map";
            outlier_cloud_last = outlierCloudLast2;
    ...
    void publishOdometry(nav_msgs::Odometry& laser_odom_to_init)
    {
        ...
        laser_odom_to_init = laserOdometry;

    }

public:
    void run(  sensor_msgs::PointCloud2 segmented_cloud,
               detail_msgs::cloud_info segmented_cloud_info,
               sensor_msgs::PointCloud2 outlier_cloud,
    ...
    {
        cloudHeader = segmented_cloud.header;
        timeScanCur = cloudHeader.stamp.toSec();
        timeNewSegmentedCloud = timeScanCur;
        segmentedCloud->clear();
        pcl::fromROSMsg(segmented_cloud, *segmentedCloud);
        newSegmentedCloud = true;

        timeNewSegmentedCloudInfo = segmented_cloud_info.header.stamp.toSec();
        segInfo = segmented_cloud_info;
        newSegmentedCloudInfo = true;

        timeNewOutlierCloud = outlier_cloud.header.stamp.toSec();
        outlierCloud->clear();
        pcl::fromROSMsg(outlier_cloud, *outlierCloud);
        newOutlierCloud = true;
        ...
        if(imuTopic != nullptr)
        {
            ...
            // 加速度去除重力影响，同时坐标轴进行变换
            float accX = imuTopic->linear_acceleration.y - sin(roll) * cos(pitch) * 9.81;
            float accY = imuTopic->linear_acceleration.z - cos(roll) * cos(pitch) * 9.81;
            float accZ = imuTopic->linear_acceleration.x + sin(pitch) * 9.81;
            ...
            imuAngularVeloX[imuPointerLast] = imuTopic->angular_velocity.x;
            imuAngularVeloY[imuPointerLast] = imuTopic->angular_velocity.y;
            imuAngularVeloZ[imuPointerLast] = imuTopic->angular_velocity.z;
        ...
        extractFeatures();
        if (!systemInitedLM) {
            checkSystemInitialization(laser_cloud_corner_last, laser_cloud_surf_last);
            return;
        }
        updateInitialGuess();
        updateTransformation();
        integrateTransformation();
        publishOdometry(laser_odom_to_init);
        publishCloudsLast(laser_cloud_corner_last, laser_cloud_surf_last, outlier_cloud_last);
    }
    void restState(nav_msgs::Odometry clean)
    {
        std::memset(transformCur,0,sizeof(float)*6);
        double roll, pitch, yaw;
        geometry_msgs::Quaternion geoQuat = clean.pose.pose.orientation;
        ...
        transformSum[3] = clean.pose.pose.position.x;
        transformSum[4] = clean.pose.pose.position.y;
        transformSum[5] = clean.pose.pose.position.z;
    }
};

...
private:
    ...
    void Invincible2(node& nowNode, vector<PointType>&truth,vector<PointType>&sim)
    {
	    //判断P（针对“当前节点与sim.front()这条识别出来的连线” 来说）
        if(!sim.empty() && sim.front().z < sameThres){  //这个P
            if(!truth.empty())  //是对的
        ...
        //判断N
        if(!truth.empty()){ //当前节点有对应真值，那么“当前节点与truth.front()这条连线”应该是有的。如果没有连，则是错误的“没连”
            if(!sim.empty() && sim.front().z < sameThres && sim.front().y > truthDisThres){ //情况1，与别的先验节点连上了
                FN_1++; 
                FPFN++;
                recogPair tempP;
                tempP.state = _F_P_F_N;
...
public:
    void run(node input_node, pcl::PointCloud<pcl::PointXYZI> input_cloud){
        ...
        Invincible2(input_node, resultDis,resultSim);
   }
};
...
int main(int argc, char** argv){
    ...
    while(ros::ok()){
        if(nowThread<MaxThread&&nowIndex < paramList.size()){
            cout << nowIndex << " add a new thread N " << paramList[nowIndex].second << " thres " << paramList[nowIndex].first << endl;
            nowThread++;

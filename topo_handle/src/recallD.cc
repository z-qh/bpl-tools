...
void callBack(double K_, double thres_){
    ...
    string loadPath = pathBase + "/buildD/D" + ss.str() + "/";
    recallTopo recallTopoMap(loadPath, thres_);
    int nodesSize = nodeList1.size();

    stringstream ss2;
    ss2 << fixed << setprecision(2) << "D" << thres_ << "R";  
    string nowDir = pathBase + "/recogD/" + ss2.str();
    if( 0 != access(nowDir.c_str(), 0) ){
        mkdir(nowDir.c_str(), 0777);
    }
    string fileResult = nowDir + "/D" + ss.str() + "RecallInfo.txt";
...
int main(int argc, char** argv){
    ...
    vector<double> buildValue{
        0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 5, 6, 7, 10, 15, 20, 25, 30
        };

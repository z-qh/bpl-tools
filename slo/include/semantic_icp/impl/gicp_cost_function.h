...
class GICPCostFunction : public ceres::SizedCostFunction<1, Sophus::SE3d::num_parameters> {
public:
    static const int K = 3;
    GICPCostFunction(const pcl::PointXYZ point_source, const pcl::PointXYZ point_target, const Eigen::Matrix3d cov_source,
                     const Eigen::Matrix3d cov_target, const Sophus::SE3<double> base_transform)
    ...
    virtual bool Evaluate(double const* const* parameters, double* residuals, double** jacobians) const {
        ...
        if (jacobians != NULL && jacobians[0] != NULL) {
            ...
            Eigen::Matrix3d dR;
            Eigen::Matrix3d Ta = (cov_target_.transpose() + R * cov_source_.transpose() * R.transpose()).inverse();
            Eigen::Vector3d tb = M * res;
            Eigen::Vector3d tc = Ta * res;

            // dR = -2.0 *( tb*point_source_.transpose()+cov_source_*R*tb*(res.transpose()*M));

            dR = -(tb * point_source_.transpose() + tc * (res.transpose() * Ta * R * cov_source_.transpose()) +
                   tb * (res.transpose() * M * R * cov_source_) + tc * point_source_.transpose());
...
private:
    ...
    // Used in Chain Rule
    inline Eigen::Quaterniond dRtodq(const Eigen::Matrix3d dR, const Eigen::Quaterniond q, const Eigen::Matrix3d R) const {

...
private:
    ...
    void copyPointCloud(const sensor_msgs::PointCloud2 laserCloudMsg){
        ...
        vector<int> nanIndex;
        pcl::removeNaNFromPointCloud(*laserCloudIn, *laserCloudIn, nanIndex);
    }

public:
    void cloudHandler(sensor_msgs::PointCloud2 velodyne_points,
                      //输出
                      sensor_msgs::PointCloud2& segmented_cloud,
                      detail_msgs::cloud_info& segmented_cloud_info,
                      sensor_msgs::PointCloud2& outlier_cloud,
                      sensor_msgs::PointCloud2& noground_cloud)
    {
        copyPointCloud(velodyne_points);
        findStartEndAngle();
        projectPointCloud();
        groundRemoval();
        cloudSegmentation();
        publishCloud(segmented_cloud, segmented_cloud_info, outlier_cloud, noground_cloud);
        resetParameters();
    }
    ...
    // 发布各类点云内容
    void publishCloud(sensor_msgs::PointCloud2& segmented_cloud,
                      detail_msgs::cloud_info& segmented_cloud_info,
                      sensor_msgs::PointCloud2& outlier_cloud,
                      sensor_msgs::PointCloud2& noground_cloud){
    	// detail_msgs::cloud_info消息
        segMsg.header = cloudHeader;
        segmented_cloud_info = segMsg;
        ...
        outlier_cloud = laserCloudTemp;

		// pubSegmentedCloud发布分块点云
        pcl::toROSMsg(*segmentedCloud, laserCloudTemp);
        laserCloudTemp.header.stamp = cloudHeader.stamp;
        laserCloudTemp.header.frame_id = "map";
        segmented_cloud = laserCloudTemp;

        // pubSegmentedCloud发布分块点云
        pcl::toROSMsg(*groundCloud, laserCloudTemp);
        laserCloudTemp.header.stamp = cloudHeader.stamp;
        laserCloudTemp.header.frame_id = "map";
        noground_cloud = laserCloudTemp;

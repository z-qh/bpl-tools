...
void SemanticPointCloud<PointT, SemanticT>::addSemanticCloud(SemanticT label, PointCloudPtr cloud_ptr, bool computeKd, bool computeCov) {
    ...
    if (computeKd == true) {
        ...
        if (computeCov == true) {
            ...
            typename pcl::PointCloud<PointT>::const_iterator points_iterator = cloud_ptr->begin();
...
template <typename PointT, typename SemanticT> pcl::PointCloud<pcl::PointXYZL>::Ptr SemanticPointCloud<PointT, SemanticT>::getpclPointCloud() {
...
template <typename PointT, typename SemanticT> void SemanticPointCloud<PointT, SemanticT>::transform(Eigen::Matrix4f trans) {
    for (SemanticT s : semanticLabels) {
        pcl::transformPointCloud(*(labeledPointClouds[s]), *(labeledPointClouds[s]), trans);
    }
};
